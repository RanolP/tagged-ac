...
export function traverseRouteTree(
  ...
  if (node.value.route) {
    const { route, page } = appendSegment(segments, node);
    node.value.route.matchSegments = route.matchSegments;
    node.value.route.params = route.params;
    node.value.route.path = page.path;
